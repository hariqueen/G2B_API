...
try:
    ...
    print(f"🎯 키워드 목록: {SEARCH_KEYWORDS}")
    print(f"📅 날짜 범위: {DEFAULT_INPUT['start_date']} ~ {DEFAULT_INPUT['end_date']}")
except Exception as e:
    print(f"❌ config import 오류: {e}")
    exit(1)

try:
    from data_processor import fetch_bid_data, process_bid_items
    print("✅ data_processor 모듈 import 성공")
except Exception as e:
    print(f"❌ data_processor import 오류: {e}")
    exit(1)

try:
    from scsbid_client import get_scsbid_amount, get_openg_corp_info, get_bid_clsfc_no, get_nobid_reason
    print("✅ scsbid_client 모듈 import 성공")
except Exception as e:
    print(f"❌ scsbid_client import 오류: {e}")
    exit(1)
...
try:
    ...
    from firebase_admin import db
    from datetime import datetime
    print("✅ firebase 모듈 import 성공")
except Exception as e:
    print(f"❌ firebase import 오류: {e}")
    exit(1)
...
def initialize_firebase():
    try:
        ...
        if firebase_credentials:
            print("✅ 환경변수에서 Firebase 인증 정보 찾음")
            # 환경 변수에 저장된 JSON 문자열을 딕셔너리로 변환
            cred_dict = json.loads(firebase_credentials)
            cred = credentials.Certificate(cred_dict)
        else:
            # 로컬 개발 환경일 경우 파일 사용
            print("🔄 로컬 Firebase 인증 파일 확인 중...")
            try:
                cred = credentials.Certificate('g2b-db-6aae9-firebase-adminsdk-fbsvc-0e3b1ce560.json')
                print("✅ 로컬 Firebase 인증 파일 찾음")
            except FileNotFoundError:
                print("❌ Firebase 인증 파일을 찾을 수 없습니다.")
                raise
...
def process_single_keyword(keyword):
    ...
    for api in BID_ENDPOINTS:
        try:
            print(f"📡 API 호출 중: {api['desc']}")
            response = fetch_bid_data(api["path"], config)
            if response is None:
                print(f"⚠️ API 응답이 없습니다: {api['desc']}")
                continue

            print(f"📦 API 응답 처리 중...")
            bid_items = process_bid_items(response.get("items", []), api["desc"], config)
            print(f"📊 처리된 입찰 항목: {len(bid_items)}개")
...
def main():
    ...
    if all_collected_data:
        ...
        print("💾 collection_result.json 파일 저장 완료")
        
        # 기존 데이터에 대한 user_inputs 생성
        try:
            create_missing_user_inputs()
        except Exception as e:
            print(f"⚠️ user_inputs 생성 중 오류: {e}")
    else:
        # 수집된 데이터가 없을 때도 파일 생성
        result_info = {
            "total_count": 0,
